...
class OutputDialog(QtWidgets.QDialog):
    ...
    def run_command(self, program, args=None, use_shell=False):
        if args is None:
            args = []
        self.proc = QtCore.QProcess(self)
        if use_shell:
            prog = '/bin/sh'
            argv = ['-c', program if isinstance(program, str) else ' '.join([program]+args)]
            self.proc.start(prog, argv)
        else:
            self.proc.start(program, args)
        self.proc.readyReadStandardOutput.connect(lambda: self.append(self.proc.readAllStandardOutput()))
        self.proc.readyReadStandardError.connect(lambda: self.append(self.proc.readAllStandardError()))
        self.exec_()
...
class TileWidget(QtWidgets.QFrame):
    ...
    def __init__(self, name, img_path=None, big=False, parent=None):
        super().__init__(parent)
        self.name = name
        if img_path:
        ...
        if self.img_path and self.img_path.exists():
            pix = QtGui.QPixmap(str(self.img_path)).scaled(400 if big else 220, 220 if big else 120, QtCore.Qt.KeepAspectRatio, QtCore.Qt.SmoothTransformation)
...
class MainWindow(QtWidgets.QMainWindow):
    def __init__(self, windowed=False):
        ...
        for name,pos in zip(others, positions):
            ...
            tw = TileWidget(name, img if img is not None else None, big=False, parent=self)
            tw.clicked.connect(self.on_tile_clicked)
            grid.addWidget(tw, pos[0], pos[1])
            self.tiles.append(tw)

        right = QtWidgets.QVBoxLayout()
        status_lbl = QtWidgets.QLabel('Battery: --%  |  Network: --')
        ...
        right.addWidget(vol_label)
        vol_slider = QtWidgets.QSlider(QtCore.Qt.Horizontal)
        vol_slider.setRange(0,100)
        vol_slider.setValue(50)
        right.addWidget(vol_slider)
        vol_btn = QtWidgets.QPushButton('Mute/Unmute')
        right.addWidget(vol_btn)
        right.addSpacing(8)

        # Brightness control
        bri_label = QtWidgets.QLabel('Brightness')
        bri_label.setStyleSheet('color:white;')
        right.addWidget(bri_label)
        bri_slider = QtWidgets.QSlider(QtCore.Qt.Horizontal)
        bri_slider.setRange(0,100)
        bri_slider.setValue(50)
        right.addWidget(bri_slider)
        right.addSpacing(8)

        # small help
        help_lbl = QtWidgets.QLabel('Quick controls: Volume / Brightness')
        ...
        def update_status():
            ...
            try:
                import glob
                caps = glob.glob('/sys/class/power_supply/*/capacity')
                states = glob.glob('/sys/class/power_supply/*/status')
                if caps:
                    c = open(caps[0]).read().strip()
                    s = open(states[0]).read().strip() if states else ''
                    bat = f"{c}% {s}"
                else:
                    # fallback to upower
                    out = subprocess.getoutput("upower -e | grep -i battery | head -n1")
                    if out:
                        op = subprocess.getoutput(f"upower -i {out} | grep -E 'percentage|state' || true")
                        bat = op.replace('\n', ' | ')
            except Exception:
                bat = 'Unknown'
            ...
            net = 'Offline'
            try:
                rout = subprocess.getoutput("ip route get 1.1.1.1 2>/dev/null | head -n1")
                if 'src' in rout:
                    net = 'Online'
                else:
                    net = 'Offline'
                ...
                net = 'Unknown'
            status_lbl.setText(f'Battery: {bat}  |  Network: {net}')

        # Connect controls
        def set_volume(val):
            try:
                subprocess.Popen(['/bin/sh','-c', f"pactl set-sink-volume @DEFAULT_SINK@ {val}%"], stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL)
            except Exception:
                pass

        def toggle_mute():
            try:
                subprocess.Popen(['/bin/sh','-c','pactl set-sink-mute @DEFAULT_SINK@ toggle'])
            except Exception:
                pass

        def set_brightness(val):
            try:
                # prefer brightnessctl, fallback to xbacklight
                if subprocess.call(['which','brightnessctl'], stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL)==0:
                    subprocess.Popen(['/bin/sh','-c', f"brightnessctl set {val}%"], stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL)
                elif subprocess.call(['which','xbacklight'], stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL)==0:
                    subprocess.Popen(['/bin/sh','-c', f"xbacklight -set {val}"], stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL)
            except Exception:
                pass

        vol_slider.valueChanged.connect(lambda v: set_volume(v))
        vol_btn.clicked.connect(lambda: toggle_mute())
        bri_slider.valueChanged.connect(lambda v: set_brightness(v))

        # periodic status update
        stat_timer = QtCore.QTimer(self)
        stat_timer.timeout.connect(update_status)
        stat_timer.start(5000)
        QtCore.QTimer.singleShot(200, update_status)
    ...
    def keyPressEvent(self, e):
        ...
        if key == QtCore.Qt.Key_Left:
            ...
            name = self.tiles[self.current_index].name
            self.on_tile_clicked(name)
            return
        else:
    ...
    def on_tile_clicked(self, name):
        ...
        if name == 'Casino':
            prog = sys.executable
            arg = os.path.join(xui, 'casino', 'casino.py')
            QtCore.QProcess.startDetached(prog, [arg])
        elif name == 'Runner':
            prog = sys.executable
            arg = os.path.join(xui, 'games', 'runner.py')
            QtCore.QProcess.startDetached(prog, [arg])
        elif name == 'Store':
            prog = '/bin/sh'
            cmd = os.path.join(xui, 'bin', 'xui_store.sh')
            QtCore.QProcess.startDetached(prog, ['-c', cmd])
        elif name == 'LAN':
            dlg = OutputDialog('LAN Info', parent=self)
            dlg.show()
            dlg.run_command('ip -4 addr show | sed -n "1,20p"', use_shell=True)
        elif name == 'Power Profile':
            opts = ['eco', 'balanced', 'performance']
            choice, ok = QtWidgets.QInputDialog.getItem(self, 'Power Profile', 'Choose profile:', opts, 1, False)
            if ok and choice:
                prog = '/bin/sh'
                script = os.path.join(xui, 'bin', 'xui_battery_profile.sh')
                QtCore.QProcess.startDetached(prog, ['-c', f"{script} {choice}"])
        elif name == 'Battery Saver':
            prog = '/bin/sh'
            script = os.path.join(xui, 'bin', 'xui_battery_saver.sh')
            QtCore.QProcess.startDetached(prog, ['-c', script])
        elif name == 'Settings':
            # simple settings: set battery threshold (persist to data file) and give option to start web control
            try:
                p = Path(XUI_HOME)/'data'/'battery_threshold'
                cur = int(p.read_text().strip()) if p.exists() else 20
            except Exception:
                cur = 20
            val, ok = QtWidgets.QInputDialog.getInt(self, 'Battery Threshold', 'Notify below (%):', cur, 5, 100, 1)
            if ok:
                try:
                    p.parent.mkdir(parents=True, exist_ok=True)
                    p.write_text(str(val))
                    QtWidgets.QMessageBox.information(self, 'Saved', f'Battery threshold set to {val}%')
                except Exception as e:
                    QtWidgets.QMessageBox.warning(self, 'Error', f'Could not save: {e}')
        elif name == 'System Info':
            dlg = OutputDialog('System Info', parent=self)
            cmd = "uname -srmo; lsb_release -a 2>/dev/null || true; free -h; lscpu | grep -E 'Model name|CPU\(s\)|Architecture' || true; df -h / | sed -n '1,5p'"
            dlg.run_command(cmd, use_shell=True)
            ...
            try:
                ...
                if screen:
                    ...
                    ts = int(time.time())
                    out = Path.home()/'.xui'/'assets'/f'screenshot-{ts}.png'
                    pix.save(str(out))
                    QtWidgets.QMessageBox.information(self, 'Screenshot', f'Saved to {out}')
                else:
                    QtWidgets.QMessageBox.warning(self, 'Screenshot', 'No screen available')
            except Exception as e:
                QtWidgets.QMessageBox.warning(self, 'Screenshot', f'Error: {e}')
        elif name == 'Web Control':
            script = os.path.join(xui, 'bin', 'xui_web_control.sh')
            if os.path.exists(script):
                QtCore.QProcess.startDetached('/bin/sh', ['-c', script])
                QtWidgets.QMessageBox.information(self, 'Web Control', 'Started web control (background)')
            else:
                QtWidgets.QMessageBox.warning(self, 'Web Control', 'Web control script not found')
        elif name == 'Update System':
            script = os.path.join(xui, 'bin', 'xui_update_system.sh')
            if os.path.exists(script):
                dlg = OutputDialog('Update System', parent=self)
                dlg.run_command(script, use_shell=True)
            else:
                QtWidgets.QMessageBox.warning(self, 'Update', 'Update script not found')
        elif name == 'Salir al escritorio':
            try:
                resp = QtWidgets.QMessageBox.question(self, 'Salir', '¿Salir al escritorio?', QtWidgets.QMessageBox.Yes | QtWidgets.QMessageBox.No)
